  ...
  >
    ...
    >
      ...
      <template v-else-if="checkDataType(item, 'array')">
        <dynamic-tags v-model="formData[propertyName(item)]" />
      </template>
...
export default {
  components: { Tinymce, DynamicTags: DynamicTags() },
  mixins: [mixin],
  props: {
    ...
    setConfig() {
      this.config.forEach((e) => {
        ...
        if (
          typeof e === 'object' &&
          'default' in e &&
          typeof e.default !== typeof value
        ) {
          this.$set(this.formData, property, e.default)
        }
